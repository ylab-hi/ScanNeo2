...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_seqs:
        with open(options.alleles, 'r') as fh_alleles:
            alleles = [allele.rstrip() for allele in fh_alleles]
        ...
        epilens = [eval(i) for i in options.lengths.split(',')]
        ...
        # Open all per-length write handles and the input together; ExitStack
        # closes (and flushes) them when the block exits, before the binding
        # affinity subprocess reads the files.
        with contextlib.ExitStack() as stack:
            ...
            fh = stack.enter_context(open(options.input, 'r'))
            next(fh)   # skip header
            for line in fh:
...
#    binding_affinities[epilen] = {}
    ...
    try:
        ...
        print(f"predict_binding.py failed (allele={allele}, epilen={epilen}): {e.stderr}", file=sys.stderr, flush=True)
        raise
