# VQSR training-resource priors (passed to the gatk/variantrecalibrator wrapper).
# Held as a module-level constant so snakefmt/black can parse the rule cleanly —
# multi-line dict literals inside a `params:` block trip snakefmt's translator.
VQSR_RESOURCES = {
...
rule get_gatk_vqsr_training_sets:
    ...
    shell:
        ...
        (
            ...
            curl --fail -L https://storage.googleapis.com/gcp-public-data--broad-references/hg38/v0/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz.tbi -o resources/vqsr/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz.tbi
        ) >{log} 2>&1
        """
...
rule recalibrate_bases:
    input:
        ...
        dict="resources/refs/genome.dict",
        known=[
            "results/{sample}/{seqtype}/indel/htcaller/{group}_indels.1rd.flt.vcf",
...
checkpoint split_bam_htc_final_round:
    ...
    output:
        directory(
            "results/{sample}/{seqtype}/indel/htcaller/{group}_variants.1rd.baserecal_split"
        ),
    log:
